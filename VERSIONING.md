...
simple-ldapd uses [Semantic Versioning](https://semver.org/) on the **0.x** series. Each completed [roadmap](ROADMAP.md) milestone is a **minor** bump. Fixes and docs inside a milestone are **patch** bumps. 1.0.0 is reserved until concurrent connections, ACLs, and hashed passwords land on top of bind, search, writes, and TLS (see [ROADMAP.md](ROADMAP.md)).
...
| **0.11.0** | 11 — Access control | Subtree search/write ACLs | Planned |
...
| **1.0.0** | Production-usable SSO directory | After 10–12 (and TLS already shipped) | Reserved |
