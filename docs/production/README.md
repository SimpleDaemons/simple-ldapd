...
v0.11.0 can bind, search, write, encrypt, enforce schema, apply ACLs, and change passwords, and it serves more than one client at a time. It is still early: lab GSSAPI rather than MIT Kerberos, cleartext `userPassword` storage, and no `stop`/`reload` commands.
