...
Version:        ${VERSION}
Release:        1%{?dist}
Summary:        simple-ldapd - A lightweight and secure daemon
License:        Apache-2.0
...
if [ "$1" -eq 1 ]; then
    ...
    # Create service user
    if ! id simple-ldapd &>/dev/null; then
        useradd -r -s /sbin/nologin -d /var/lib/%{name} -c "%{name} service user" simple-ldapd
    fi
fi
