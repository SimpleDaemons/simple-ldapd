...
# simple-ldapd macOS Build Script
# This script builds the simple-ldapd application for macOS
...
build_project() {
    print_status "Building simple-ldapd..."
...
install_project() {
    if [ "$INSTALL" = "true" ]; then
        print_status "Installing simple-ldapd..."
        cd "$BUILD_DIR"
        
        make install
        
        # Test installation
        if command_exists simple-ldapd; then
...
create_static_package() {
    if [ "$ENABLE_STATIC_LINKING" = "ON" ]; then
        ...
        STATIC_DIR="$DIST_DIR/simple-ldapd-$VERSION-static-macos"
        mkdir -p "$STATIC_DIR"
        
        # Copy binary and files
        cp "$BUILD_DIR/simple-ldapd" "$STATIC_DIR/"
        ...
        # Create tarball
        cd "$DIST_DIR"
        tar -czf "simple-ldapd-$VERSION-static-macos.tar.gz" "simple-ldapd-$VERSION-static-macos"
        rm -rf "simple-ldapd-$VERSION-static-macos"
        
        print_success "Static binary package created: simple-ldapd-$VERSION-static-macos.tar.gz"
    fi
}
...
# Main execution
main() {
    print_status "Starting simple-ldapd build process..."
    ...
    print_success "simple-ldapd build process completed successfully!"
}
